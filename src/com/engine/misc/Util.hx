...
public static inline function getScaled(source:BitmapData, newWidth:Int, newHeight:Int):BitmapData 
	{
	   var bmp:BitmapData = new BitmapData(newWidth, newHeight, false,0);
		bmp.copyPixels( source, source.rect, new openfl.geom.Point(), null, null, true );
		return bmp;
	}
...
var bmp:BitmapData = new BitmapData(newWidth, newHeight, false,  0x000000);
bmp.draw(source, m);
source.dispose();
return bmp;
}
...
public static inline function saveBitmapData(data:BitmapData,filename:String)
{
  ...
  var b:ByteArray =data.encode(data.rect,new PNGEncoderOptions (), null);
